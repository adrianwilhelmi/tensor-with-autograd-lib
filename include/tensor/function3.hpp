...
class Function{
public:
	Function(const Tensor<T>& at, const Tensor<T>& bt, 
			const Tensor<T>&og){
		if(at.requires_grad()){
			this->a = at.get_grad_fn();
		}
		if(bt.requires_grad()){
			this->b = bt.get_grad_fn();
		}
		og.storage().share(this->data);
	}
...
protected:
	std::shared_ptr<func_variant<T>> a;
	std::shared_ptr<func_variant<T>> b;
	Storage<T> data;
	Tensor<T> grads;
...
public:
	...
	void backward_impl(const Tensor<T>& grad){
		if(this->a.get().requires_grad()){
			this->a.get().grad(grad.descriptor()) += grad;
